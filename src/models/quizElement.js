...
class QuizElement {
    ...
    static async getQuizElement(id) {
        try {
            const response = await pool.query("SELECT * FROM quizelement WHERE idquizelement = $1", [id]);
            return response.rows[0];
        } catch (error) {
            throw new Error("Error getting quizElement");
        }
    }
