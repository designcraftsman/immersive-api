...
class QuizElementAnswer {
    ...
    static async getQuizElementAnswer(id) {
        try {
            const response = await pool.query("SELECT * FROM quizelementanswer WHERE idquizelementanswer = $1", [id]);
            return response.rows[0];
        } catch (error) {
            throw new Error("Error getting quizElementAnswer");
        }
    }
